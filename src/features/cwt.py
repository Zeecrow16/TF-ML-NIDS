...
class CalculateCwt:
    """
    Class to compute CWT features (mean magnitude & mean phase) for UNSW-NB15 dataset.
    """
    ...
    def calculate_cwt_category(self, df: pd.DataFrame, columns: List[str]):
        ...
        results = np.array([self.cwt_mean_features(row, self.scales, self.wavelet) for row in signals])
        mean_mag = results[:, 0]
        mean_phase = results[:, 1]
        return mean_mag, mean_phase

    def calculate_all_categories(self, df: pd.DataFrame, signal_categories: Dict[str, List[str]]):
        """
        Apply CWT to all feature groups in df and add mean/phase columns.
        """
        df_cwt = df.copy()
