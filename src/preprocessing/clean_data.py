...
class UNSWNB15Dataset:
    ...
    def __init__(self, csv_path: str):
        ...
        self.throughput_signal = ["SRC_TO_DST_AVG_THROUGHPUT", "DST_TO_SRC_AVG_THROUGHPUT"]
    ...
    def clean_data(self):
        ...
        self.df_clean = self.df[self.cwt_columns + ["Attack"]].replace([np.inf, -np.inf], np.nan).dropna()
    ...
    def sample_data(
        ...
        benign_sample = benign_df.sample(
            n=min(len(benign_df), benign_target),
        ...
        self.df_sampled = pd.concat([benign_sample, attack_sample]).sample(frac=1, random_state=random_state).reset_index(drop=True)
